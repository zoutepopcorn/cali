...
echo "options cryptdevice=`blkid | grep crypto_LUKS | awk '{print $6}' | sed 's/"//g'`:cryptlvm crypto=sha512:aes-xts-plain64:512:0: root=/dev/mapper/vg0-lv_root resume=/dev/mapper/vg0-lv_swap" >> /boot/loader/entries/arch.conf
...
echo "options cryptdevice=`blkid | grep crypto_LUKS | awk '{print $6}' | sed 's/"//g'`:cryptlvm crypto=sha512:aes-xts-plain64:512:0: root=/dev/mapper/vg0-lv_root resume=/dev/mapper/vg0-lv_swap" >> /boot/loader/entries/arch-lts.conf
